"""Compact U-Net architectures for RGB image-to-image deblending."""
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        output = self.activation(self.norm1(self.conv1(x)))
...
class ResUNet(nn.Module):
    """Small residual-block U-Net that predicts an unconstrained RGB residual.

    The encoder/decoder topology and channel widths match :class:`UNet`; only
    the internal two-convolution blocks are residual. The final activation is
    intentionally the identity because deblending reconstructs the target as
    ...
    def __init__(
        ...
        super().__init__()
        self.enc1 = ResidualBlock(in_channels, base_channels, norm=norm)
        self.pool1 = nn.MaxPool2d(2)
        self.enc2 = ResidualBlock(base_channels, base_channels * 2, norm=norm)
        self.pool2 = nn.MaxPool2d(2)
        self.enc3 = ResidualBlock(base_channels * 2, base_channels * 4, norm=norm)
        self.pool3 = nn.MaxPool2d(2)
